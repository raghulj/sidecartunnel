...
// carries (FR-23).
//
// It returns the reason to count the rejection under, so that
// st_control_rejected_total{reason} distinguishes the three cases an operator acts on
// differently: unsigned is somebody publishing to the control channel who should not be,
// stale is this replica's clock or a genuinely old message, malformed is an application
// bug in whatever built it.
//
// The signature is checked before the timestamp, deliberately. The signature covers the
// timestamp, so a forgery cannot be counted as "stale" — a stale count therefore always
// means an authentic message outside the window, which is the count worth alerting on.
...
func verifyControl(secret []byte, now time.Time, payload []byte) (hub.Control, metrics.ControlReason, error) {
	var env controlEnvelope
	if err := json.Unmarshal(payload, &env); err != nil {
		return hub.Control{}, metrics.ControlMalformed,
			fmt.Errorf("%w: envelope is not a JSON object: %w", errControl, err)
	}

	sig, err := hex.DecodeString(env.Sig)
	if err != nil {
		return hub.Control{}, metrics.ControlUnsigned,
	...
	if skew := now.Sub(time.Unix(env.TS, 0)); skew > controlSkew || skew < -controlSkew {
		return hub.Control{}, metrics.ControlStale,
	...
	if err != nil {
		return hub.Control{}, metrics.ControlMalformed, fmt.Errorf("%w: %w", errControl, err)
	}
	return c, "", nil
}
