...
| Signatures | `checksums.txt.bundle`, cosign keyless (`.sig` + `.pem` up to v0.1.1) |
...
The **cosign binary is pinned in `release.yml`**, at v3.0.6, rather than inherited from
whatever `cosign-installer` currently defaults to. The tool that produces the signatures
decides what the release publishes, so which version runs is a decision recorded in the
workflow and not a consequence of a dependency bump.
...
Container signatures could also have moved, from the `.sig` tag layout to OCI 1.1
referring artifacts. That one is **held at legacy on purpose**, with
`--registry-referrers-mode=legacy` written explicitly in `docker_signs`. It changes where
every verifier has to look, including verifiers running older cosign, and it would have
silently invalidated the `cosign verify` command already published for `v0.1.1`. Moving it
is a decision with its own compatibility question; it should not arrive as a side effect
of upgrading a binary.
